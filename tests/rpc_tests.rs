...
//!
//! These tests only run on Unix systems since Unix sockets are not available on Windows.
...
fn test_rpc_request_with_params() {
    let json =
        r#"{"jsonrpc":"2.0","method":"player.play","params":{"uri":"spotify:track:123"},"id":2}"#;
    let req: RpcRequest = serde_json::from_str(json).expect("Failed to parse request");
...
async fn test_server_multiple_requests() {
    ...
    // Second request
    writer
        .write_all(b"{\"jsonrpc\":\"2.0\",\"method\":\"version\",\"id\":2}\n")
        .await
        .unwrap();
    writer.flush().await.unwrap();
...
fn test_rpc_response_from_success_with_payload() {
    ...
    let cli_response =
        Response::success_with_payload(200, "Data retrieved", serde_json::json!({"key": "value"}));
