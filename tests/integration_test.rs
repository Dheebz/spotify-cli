...
fn test_resource_type_from_str() {
    assert_eq!(
        "track".parse::<ResourceType>().unwrap(),
        ResourceType::Track
    );
    assert_eq!(
        "album".parse::<ResourceType>().unwrap(),
        ResourceType::Album
    );
    assert_eq!(
        "artist".parse::<ResourceType>().unwrap(),
        ResourceType::Artist
    );
    assert_eq!(
        "playlist".parse::<ResourceType>().unwrap(),
        ResourceType::Playlist
    );
    assert_eq!("show".parse::<ResourceType>().unwrap(), ResourceType::Show);
    assert_eq!(
        "episode".parse::<ResourceType>().unwrap(),
        ResourceType::Episode
    );
    assert_eq!(
        "audiobook".parse::<ResourceType>().unwrap(),
...
use spotify_cli::types::{Album, Artist, Device, PlaybackState, Track};
...
fn test_playback_state_progress_formatting() {
    ...
    for (progress, expected) in cases {
        let json = base_json.replace(
            "is_playing\":true",
            &format!("is_playing\":true,\"progress_ms\":{}", progress),
        );
...
fn test_response_json_format() {
    let resp = Response::success_with_payload(200, "Test", serde_json::json!({"key": "value"}));
    let json = resp.to_json();
