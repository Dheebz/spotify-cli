    ...
    pub fn path(&self) -> String {
        match self {
            ...
            Endpoint::AudiobookChapters { id, limit, offset } => {
                format!(
                    "/audiobooks/{}/chapters?limit={}&offset={}",
                    id, limit, offset
                )
            }
...
mod tests {
    ...
    fn player_endpoints() {
        ...
        assert_eq!(
            Endpoint::PlayerCurrentlyPlaying.path(),
        ...
        assert_eq!(
            Endpoint::PlayerRecentlyPlayed.path(),
    ...
    fn track_endpoints() {
        assert_eq!(
            Endpoint::Track { id: "track123" }.path(),
            "/tracks/track123"
        );
        assert_eq!(
            Endpoint::Tracks { ids: "t1,t2,t3" }.path(),
            "/tracks?ids=t1,t2,t3"
        );
    }
    ...
    fn album_endpoints() {
        assert_eq!(
            Endpoint::Album { id: "album123" }.path(),
            "/albums/album123"
        );
        assert_eq!(
            Endpoint::Albums { ids: "a1,a2" }.path(),
            "/albums?ids=a1,a2"
        );
        assert_eq!(
    ...
    fn artist_endpoints() {
        assert_eq!(
            Endpoint::Artist { id: "artist123" }.path(),
            "/artists/artist123"
        );
        assert_eq!(
            Endpoint::Artists { ids: "a1,a2" }.path(),
            "/artists?ids=a1,a2"
        );
        assert_eq!(
            Endpoint::ArtistTopTracks {
                id: "artist123",
                market: "US"
            }
            .path(),
    ...
    fn follow_endpoints() {
        ...
        assert_eq!(
            Endpoint::FollowPlaylistContains {
                playlist_id: "pl123",
                ids: "user1"
            }
            .path(),
    ...
    fn episode_endpoints() {
        ...
        assert_eq!(
            Endpoint::Episodes { ids: "e1,e2" }.path(),
            "/episodes?ids=e1,e2"
        );
        assert_eq!(
            Endpoint::SavedEpisodes {
                limit: 20,
                offset: 0
            }
            .path(),
    ...
    fn audiobook_endpoints() {
        assert_eq!(
            Endpoint::Audiobook { id: "ab123" }.path(),
            "/audiobooks/ab123"
        );
        assert_eq!(
            Endpoint::Audiobooks { ids: "ab1,ab2" }.path(),
            "/audiobooks?ids=ab1,ab2"
        );
        assert_eq!(
    ...
    fn chapter_endpoints() {
        ...
        assert_eq!(
            Endpoint::Chapters { ids: "c1,c2" }.path(),
            "/chapters?ids=c1,c2"
        );
    }
