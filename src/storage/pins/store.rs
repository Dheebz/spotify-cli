...
impl PinStore {
    ...
    pub fn add(&mut self, pin: Pin) -> Result<(), PinError> {
        ...
        if self
            .pins
            .iter()
            .any(|p| p.alias.to_lowercase() == pin.alias.to_lowercase())
        {
...
mod tests {
    ...
    fn pin_store_remove_by_alias() {
        ...
        store
            ...
            .unwrap();
        store
            .add(make_pin("chill", "track456", ResourceType::Track, vec![]))
            .unwrap();
    ...
    fn pin_store_list_with_filter() {
        ...
        store
            .add(make_pin("track1", "t1", ResourceType::Track, vec![]))
            .unwrap();
        store
            .add(make_pin("playlist1", "p1", ResourceType::Playlist, vec![]))
            .unwrap();
        store
            .add(make_pin("track2", "t2", ResourceType::Track, vec![]))
            .unwrap();
    ...
    fn pin_store_all() {
        ...
        store
            .add(make_pin("pin1", "id1", ResourceType::Track, vec![]))
            .unwrap();
        store
            .add(make_pin("pin2", "id2", ResourceType::Album, vec![]))
            .unwrap();
    ...
    fn pin_store_persistence() {
        ...
        // Create and add pins
        {
            ...
            store
                .add(make_pin("persistent", "id123", ResourceType::Track, vec![]))
                .unwrap();
        }
    ...
    fn pin_store_loads_existing_file() {
        ...
        writeln!(
            file,
            r#"{{"pins":[{{"resource_type":"track","id":"abc","alias":"test","tags":[]}}]}}"#
        )
        .unwrap();
