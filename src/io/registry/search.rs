...
impl PayloadFormatter for SpotifySearchFormatter {
    ...
    fn matches(&self, payload: &Value) -> bool {
        ...
        !is_playlist
            && !is_album
            && ((payload.get("tracks").is_some() && payload["tracks"].get("items").is_some())
                || (payload.get("albums").is_some() && payload["albums"].get("items").is_some())
                || (payload.get("artists").is_some() && payload["artists"].get("items").is_some())
...
mod tests {
    ...
    fn combined_search_supports_kind() {
        ...
        assert!(
            formatter
                .supported_kinds()
                .contains(&PayloadKind::CombinedSearch)
        );
    }
    ...
    fn spotify_search_supports_kind() {
        ...
        assert!(
            formatter
                .supported_kinds()
                .contains(&PayloadKind::SearchResults)
        );
    }
