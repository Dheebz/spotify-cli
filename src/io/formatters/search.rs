...
pub fn format_spotify_search(payload: &Value, has_results: &mut bool) {
    ...
    {
        *has_results = true;
        let rows: Vec<Vec<String>> = artists
            .iter()
            .map(|artist| {
                let name = artist
                    .get("name")
                ...
                vec![truncate(name, 30), followers, get_score(artist).to_string()]
            })
            .collect();
        print_table(
            "Artists",
        ...
        && !shows.is_empty()
    {
        *has_results = true;
        ...
        print_table(
            "Shows",
            &["Name", "Publisher", "Episodes", "Score"],
            &rows,
            &[30, 20, 8, 5],
        );
    }

    if let Some(episodes) = payload
        .get("episodes")
        .and_then(|t| t.get("items"))
        .and_then(|i| i.as_array())
        && !episodes.is_empty()
    {
        *has_results = true;
        ...
        print_table(
            "Episodes",
            &["Name", "Show", "Duration", "Score"],
            &rows,
            &[30, 20, 8, 5],
        );
    }

    if let Some(audiobooks) = payload
        .get("audiobooks")
        .and_then(|t| t.get("items"))
        .and_then(|i| i.as_array())
        && !audiobooks.is_empty()
    {
        *has_results = true;
        let rows: Vec<Vec<String>> = audiobooks
            .iter()
            .map(|book| {
                let name = book
                    .get("name")
